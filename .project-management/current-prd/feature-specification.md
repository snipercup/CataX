...
The goal of this feature is to add a button just like the null_tile button that allows the user to add a npc_tile to the brushcomposer. The user can then use the brushcomposer to paint this tile onto the map. The NPC tile can exist together with normal tiles, but not together with `mob`, `mobgroup`, `furniture` and `itemgroup`. If an NPC tile is painted onto a coordinate that already has one of those, it should be replaced by the NPC tile. 
