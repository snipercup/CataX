...
func _create_recipe(level: int, bonus_stat: String = "") -> RItem.CraftRecipe:
	var data := {
		"skill_requirement": {"id": "fabrication", "level": level}, "skill_bonus_stat": bonus_stat
	}
...
func test_has_required_skill_without_bonus_stat():
	...
	assert_false(
		CraftingRecipesManager.has_required_skill(recipe), "Crafting should fail without bonus stat"
	)
...
func test_has_required_skill_with_bonus_stat():
	...
	assert_true(
		CraftingRecipesManager.has_required_skill(recipe), "Crafting should succeed with bonus stat"
	)
