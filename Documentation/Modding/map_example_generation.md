...
| `Tools/examples/map_recipe_building_surfaces.json` | `Mods/Dimensionfall/Maps/generated_building_surfaces.json` | One validated building with its complete enclosed office classified as interior and `covered_open` garage classified as open space; an opt-in complete room partition requires every owned room be classified exactly once. Both retain authored exterior access, while the building has `roof` and `ceiling` classifications at logical `z: 1` plus an opt-in composition requiring both without generated overhead geometry. |
...
For the maintained building-surfaces map, open `generated_building_surfaces`, save it, close it, and re-open it. Confirm `office_building` retains `access_validation: "complete"`, `interior_rooms: ["office"]`, `open_space_rooms: ["garage_bay"]`, and `room_partition_validation: "complete"`. The office and garage are therefore the exact authored partition of the building’s owned rooms. Their existing `office_front_door` and `garage_opening` room-to-exterior links independently satisfy authored access completeness. Confirm the root `building_surfaces` array retains `office_roof` (`roof`) and `office_ceiling` (`ceiling`), both bound to the `[7, 7]` `5×4` building footprint at logical `z: 1`, and that `building_compositions` retains `office_complete_overhead` requiring both kinds. The seven `brick_wall_00` tiles plus both existing `door_wood` features must remain unchanged. **Save and test** must not add roof/ceiling tiles, meshes, collision, support, lighting, weather, indoor state, pathfinding, or any runtime behavior; this slice verifies authored metadata persistence and cross-record consistency only.
