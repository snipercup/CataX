...
| `Tools/examples/map_recipe_building_surfaces.json` | `Mods/Dimensionfall/Maps/generated_building_surfaces.json` | One validated `office_building` whose complete enclosed office is explicitly classified as interior, with authored complete access to exterior, `roof` and `ceiling` classifications at its immediately overhead logical `z: 1`, plus an opt-in composition requiring both classifications without generated overhead geometry. |
