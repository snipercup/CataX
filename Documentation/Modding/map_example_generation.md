...
| `Tools/examples/map_recipe_building_surfaces.json` | `Mods/Dimensionfall/Maps/generated_building_surfaces.json` | One validated building with a complete enclosed office explicitly classified as interior and a `covered_open` garage explicitly classified as open space; both retain authored exterior access, while the building has `roof` and `ceiling` classifications at logical `z: 1` plus an opt-in composition requiring both without generated overhead geometry. |
