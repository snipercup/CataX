...
For the maintained furnished clearing, confirm that the editor shows a garden bench at `[16, 16]`, an unlit campfire at `[15, 16]`, and a potted plant at `[17, 16]`. It deterministically scatters 24 trees or burned stumps, then 16 rocks or wild-vegetation features, over the 16×16 clearing while leaving those three authored features untouched. The maintainer has inspected the prior tree/decor clearing in the editor with `save and test` and verified all its features. Re-run that inspection after generating this version and specifically check the new rock and vegetation entries. `rock_field_00` uses `rock_field_placeholder_burned_stump_64.png` and `wild_vegetation_00` uses `wild_vegetation_placeholder_potted_plant_32.png`: each is a deliberately named copy of existing furniture art until dedicated sprites replace it. The recipe intentionally leaves `itemgroups` empty and does not exercise container contents, multi-cell occupancy, or cross-level support rules.
...
- Generated maps support terrain plus explicit known single-cell furniture and deterministic weighted furniture scatter over eligible terrain cells. The maintained clearing includes temporary placeholder-backed rock and wild-vegetation definitions; dedicated sprites remain follow-up content work. Itemgroup contents, multi-tile or tall features, automatic support inference, areas, buildings, semantic roads, and multi-level templates are not supported yet.
