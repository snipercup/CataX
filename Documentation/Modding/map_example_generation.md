...
| `Tools/examples/map_recipe_building_surfaces.json` | `Mods/Dimensionfall/Maps/generated_building_surfaces.json` | One validated `office_building` with complete authored access to exterior, `roof` and `ceiling` classifications at its immediately overhead logical `z: 1`, plus an opt-in composition that requires both classifications without generated overhead geometry. |
