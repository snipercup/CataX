						...
						if tileJSON.has("id") and tileJSON.id != "":
							if tileJSON.has("mob"):
								...
								processed_leveldata.mobs.append(
									{"json": tileJSON.mob, "pos": Vector3(w, y + 1.0, h) + offset}
								)
							if tileJSON.has("mobgroup"):
								...
								var random_mob_id: String = (
									Runtimedata.mobgroups.by_id(mobgroup_id).get_random_mob_id()
								)
								if random_mob_id != "":
